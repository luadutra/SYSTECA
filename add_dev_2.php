...
<?php $passada = filter_input(INPUT_POST, 'aluno', FILTER_SANITIZE_STRING); ?>
...
<form method="post" action="grava_dev.php?aux=<?php echo $passada;?>">
    
<div class="form-row" align="center">    
    <div class="form-group" align="center">
          <div align="center"><label for="inputDis"><b>SELECIONE O LIVRO</b></label></div>
     <td>
     	<select class="meuselect" name="livro" required>
            <option value="" style="text-align:center"> -- </option>
        	<?php
				$busca = "SELECT * FROM retirada WHERE alu_cod = '$passada' AND situacao = 1 ORDER BY livro ASC";
				$resultado = mysqli_query($conexao, $busca);
				while($rows_busca = mysqli_fetch_assoc($resultado)) {
					$data = date('d-m-Y', strtotime($rows_busca['data_ret']));
			?>
            <option value="<?php echo $rows_busca['codigo_liv']; ?>">RETIRADO EM:  <?php echo $data; ?> ________ <?php echo $rows_busca['livro']; ?> </option>
            <?php } ?>
        </select>
     </td>  
    </div>
...
<div align="center">
<button type="submit" class="btn btn-success" name="botao">FINALIZAR</button>
<hr>                  
<button type="button" class="btn btn-primary btn-sm" onClick="window.location='add_dev.php';">SELECIONAR OUTRO ALUNO</button>
<br><br>
