...
if(isset($_POST['buscar'])) {
    ...
    $busca = "SELECT * FROM aluno WHERE CONCAT(nome, serie) LIKE '%".$valorBusca."%' ORDER BY nome ASC";
    $buscar_resultado = filterTable($conexao, $busca);
    
} else {
    $busca = "SELECT * FROM aluno ORDER BY nome ASC";
    $buscar_resultado = filterTable($conexao, $busca);
}
...
<input autocomplete="off" style="text-align:center; font-weight:bold" name="nome" type="text" class="form-control" id="nome_alu" value="ALUNOS CADASTRADOS: <?php echo $quant; ?>" readonly>
</div>
<hr>
</div>

<div align="center">
	<form class="form-inline md-form mr-auto mb-4" action="lista_alu.php" method="post">
...
<table class="table table-bordered" style="font-size:15px">
  <thead class="thead-dark" align="center">
    <tr>
      ...
      <th style="font-size:14px" scope="col"><div align="center">EM ABERTO</div></th>
      <th style="font-size:14px" scope="col"><div align="center">RETIRADOS</div></th>
      <th style="font-size:14px" scope="col"><div align="center">ATRASOS</div></th>
...
<?php while($rows_busca = mysqli_fetch_assoc($buscar_resultado)) { ?>
      ...
      <td style="font-size:14px"><div align="center"><?php echo $rows_busca['aberto'];?></div></td>
      <td style="font-size:14px"><div align="center"><?php echo $rows_busca['retiradas'];?></div></td>
      <td style="font-size:14px"><div align="center"><?php echo $rows_busca['atrasos'];?></div></td>
      <td style="font-size:14px"><div align="center">
<button style="font-size:12px" type="button" class="btn btn-warning btn-sm" onClick="window.location='alt_alu.php?aux=<?php echo $rows_busca['alu_cod'];?>';">EDITAR</button>
<button style="font-size:12px" type="button" class="btn btn-danger btn-sm" onClick="window.location='exc_alu.php?aux=<?php echo $rows_busca['alu_cod'];?>';">EXCLUIR</button>
    </div></td>
    </tr>
